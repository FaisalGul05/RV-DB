import rv1 from "../Components/Svg/rv1.svg";
import rv2 from "../Components/Svg/rv2.svg";
import rv3 from "../Components/Svg/rv3.svg";
import km from "../Components/Svg/km.svg";
import cc from "../Components/Svg/cc.svg";
import transmission from "../Components/Svg/transmission.svg";
import RvImage from "../Components/Svg/RvImageDetails2.svg";
import RvImageTwo from "../Components/Svg/RvGalleryTwo.svg";
import RvImage2 from "../Components/Svg/RvGalleryOne.svg";
import MakeIcon from "../Components/Svg/Make.svg";
import LPGIcon from "../Components/Svg/LPG.svg";
import BlackWaterIcon from "../Components/Svg/BlackWater.svg";
import FreshWaterIcon from "../Components/Svg/FreshWater.svg";
import GrayWaterIcon from "../Components/Svg/GrayWater.svg";
import HotWaterIcon from "../Components/Svg/HotWater.svg";
import GeneratorIcon from "../Components/Svg/Generator.svg";
import ChassisIcon from "../Components/Svg/Chassis.svg";
import ModelYearIcon from "../Components/Svg/ModelYear.svg";
import MSRFIcon from "../Components/Svg/MSRF.svg";
import LengthIcon from "../Components/Svg/Length.svg";
import FuelIcon from "../Components/Svg/Fuel.svg";
import FuelCapacityIcon from "../Components/Svg/FuelCapacity.svg";
import TanklessWaterIcon from "../Components/Svg/TanklessWater.svg";
import SeatbeltsIcon from "../Components/Svg/Seatbelts.svg";
import SleepsIcon from "../Components/Svg/Sleeps.svg";

import { Rating } from "@mui/material";
export const heroSectionDropDownData = [
  {
    name: "Model",
    text: `Select Model`,

    options: [
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
    ],
  },
  {
    name: "Manufacturer",
    text: "Select Manufacturer",
    options: [
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
    ],
  },
  {
    name: "Min Year",
    text: "Select Min Year",
    options: [
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
    ],
  },
  {
    name: "Max Year",
    text: "Select Max Year",
    options: [
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
    ],
  },
  {
    name: "Min Length",
    text: "Select Min Length",
    options: [
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
    ],
  },
  {
    name: "Max Length",
    text: "Select Max Length",
    options: [
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
    ],
  },
  {
    name: "Min Generator Wattage",
    text: `Select Min Generator Wattage`,

    options: [
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
    ],
  },
  {
    name: "Max Generator Wattage",
    text: `Select Max Generator Wattage`,

    options: [
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
    ],
  },
  {
    name: "Chassis",
    text: `Select Chassis`,
    options: [
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
    ],
  },
  {
    name: "Features",
    text: `Select Features`,
    options: [
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
    ],
  },
];

export const heroSectionDropDownData2 = [
  {
    name: "Min Generator Wattage",
    text: `Select Min Generator Wattage`,

    options: [
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
    ],
  },
  {
    name: "Max Generator Wattage",
    text: `Select Max Generator Wattage`,

    options: [
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
      { name: "option" },
    ],
  },
];

export const cardData = [
  {
    id: 1,
    image: rv1,

    name: "Rv Name here",
    price: "Price here",
    text: "Lorem Ipsum is simply dummy text of the printing and typesetting industry",
    carConfig: [
      { image: km, value: "km here" },
      { image: cc, value: "cc here" },
      { image: transmission, value: "manual" },
    ],
  },
  {
    id: 2,
    image: rv2,

    name: "Rv Name here",
    price: "Price here",
    text: "Lorem Ipsum is simply dummy text of the printing and typesetting industry",
    carConfig: [
      { image: km, value: "km here" },
      { image: cc, value: "cc here" },
      { image: transmission, value: "manual" },
    ],
  },
  {
    id: 3,
    image: rv3,

    name: "Rv Name here",
    price: "Price here",
    text: "Lorem Ipsum is simply dummy text of the printing and typesetting industry",
    carConfig: [
      { image: km, value: "km here" },
      { image: cc, value: "cc here" },
      { image: transmission, value: "manual" },
    ],
  },
  {
    id: 4,
    image: rv1,

    name: "Rv Name here",
    price: "Price here",
    text: "Lorem Ipsum is simply dummy text of the printing and typesetting industry",
    carConfig: [
      { image: km, value: "km here" },
      { image: cc, value: "cc here" },
      { image: transmission, value: "manual" },
    ],
  },
  {
    id: 5,
    image: rv2,

    name: "Rv Name here",
    price: "Price here",
    text: "Lorem Ipsum is simply dummy text of the printing and typesetting industry",
    carConfig: [
      { image: km, value: "km here" },
      { image: cc, value: "cc here" },
      { image: transmission, value: "manual" },
    ],
  },
  {
    id: 6,
    image: rv3,

    name: "Rv Name here",
    price: "Price here",
    text: "Lorem Ipsum is simply dummy text of the printing and typesetting industry",
    carConfig: [
      { image: km, value: "km here" },
      { image: cc, value: "cc here" },
      { image: transmission, value: "manual" },
    ],
    link: "/",
  },
];

export const ReviewTableData = [
  {
    ID: 1,
    Name: "Holiday Rambler - Invicta 32Rw",
    Length: `39' 10"/ 12 METERS `,
    Year: "2022",
    Overall: 4,
    Bathroom: 3,
    Kitchen: 2,
    Interior: 4,
    Exterior: 5,
    Review: "I love this RV",
  },
  {
    ID: 2,
    Name: "Holiday Rambler - Invicta 32Rw",
    Length: `39' 10"/ 12 METERS `,
    Year: "2022",
    Overall: 4,
    Bathroom: 3,
    Kitchen: 2,
    Interior: 4,
    Exterior: 5,
    Review: "I love this RV",
  },
  {
    ID: 3,
    Name: "Holiday Rambler - Invicta 32Rw",
    Length: `39' 10"/ 12 METERS `,
    Year: "2022",
    Overall: 4,
    Bathroom: 3,
    Kitchen: 2,
    Interior: 4,
    Exterior: 5,
    Review: "I love this RV",
  },
];

export const RvImages = [RvImage, RvImage2, RvImageTwo, RvImage2];

export const DescriptionTableColumn1 = [
  {
    icon: MakeIcon,
    text: "Holiday Rambler - Invicta 32RV",
    heading: "Make and Manufacture",
  },
  { icon: LPGIcon, text: "106 lb / 48.1 kgs", heading: "LPG Capacity" },
  {
    icon: BlackWaterIcon,
    text: "50 Gallons / 189.2 litres",
    heading: "Black Water",
  },
  {
    icon: FreshWaterIcon,
    text: "100 Gallons / 189.2 litres",
    heading: "Fresh Water",
  },
  {
    icon: GrayWaterIcon,
    text: "100 Gallons / 189.2 litres",
    heading: "Gray Water",
  },
  {
    icon: HotWaterIcon,
    text: "100 Gallons / 189.2 litres",
    heading: "Hot Water Capacity",
  },
  {
    icon: GeneratorIcon,
    text: "5500 Generator",
    heading: "Generator",
  },
  {
    icon: ChassisIcon,
    text: "Ford Motorhome Chassis ",
    heading: "Chassis",
  },
];

export const DescriptionTableColumn2 = [
  { icon: ModelYearIcon, text: "2022", heading: "Model Year" },
  { icon: MSRFIcon, text: "9000,275", heading: "MSFR" },
  { icon: LengthIcon, text: `34' 78" / 10.5 meters`, heading: "Length" },
  { icon: FuelIcon, text: "Gas", heading: "Fuel Type" },
  {
    icon: FuelCapacityIcon,
    text: "100 Gallons / 189.2 litres",
    heading: "Fuel Capacity",
  },
  { icon: TanklessWaterIcon, text: "No", heading: "Tankless Hot Water Heater" },
  { icon: SeatbeltsIcon, text: "0", heading: "Seatbelts" },
  { icon: SleepsIcon, text: "7", heading: "Sleeps" },
];


export const RVCarouselRatings = [
  {
    name: 'Name Here',
    rating: [
      {
        title: 'Overall',
        rating: 5
      },
      {
        title: 'Bathroom',
        rating: 4
      },
      {
        title: 'Kitchen',
        rating: 5
      },
      {
        title: 'Interior',
        rating: 5
      },
      {
        title: 'Exterior',
        rating: 5
      },
    ]
  },
  {
    name: 'Name Here',
    rating: [
      {
        title: 'Overall',
        rating: 5
      },
      {
        title: 'Bathroom',
        rating: 4
      },
      {
        title: 'Kitchen',
        rating: 5
      },
      {
        title: 'Interior',
        rating: 5
      },
      {
        title: 'Exterior',
        rating: 5
      },
    ]
  },
  {
    name: 'Name Here',
    rating: [
      {
        title: 'Overall',
        rating: 5
      },
      {
        title: 'Bathroom',
        rating: 4
      },
      {
        title: 'Kitchen',
        rating: 5
      },
      {
        title: 'Interior',
        rating: 5
      },
      {
        title: 'Exterior',
        rating: 5
      },
    ]
  },
  {
    name: 'Name Here',
    rating: [
      {
        title: 'Overall',
        rating: 5
      },
      {
        title: 'Bathroom',
        rating: 4
      },
      {
        title: 'Kitchen',
        rating: 5
      },
      {
        title: 'Interior',
        rating: 5
      },
      {
        title: 'Exterior',
        rating: 5
      },
    ]
  },

]